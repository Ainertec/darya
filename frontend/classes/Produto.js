...
function gerarListaDeProdutos(json) {
  ...
  codigoHTML += `<tr>
        <td class="table-warning text-dark"><strong><span class="fas fa-hamburger"></span> ${
          json.name
        }</strong></td>
        ...
        <td class="table-warning"><button onclick="carregarDadosProduto('${
          json._id
        }')" type="button" class="btn btn-primary btn-sm"><span class="fas fa-edit"></span> Editar</button></td>
        <td class="table-warning"><button onclick="excluirProduto('${
          json._id
...
function testeProduto(tipo) {
  if (tipo == 'donalds') {
    return JSON.parse(
      `[{"_id":"idj52","nome":"Donalds Chocolate","descricao":"Sabor chocolate","precovenda":2.50,"precocusto":0.50,"quantidade":100}]`
    );
  } else {
    return JSON.parse(
      `[{"_id":"rdg63","nome":"Donalds Baunilha","descricao":"Sabor Baunilha","precovenda":2.50,"precocusto":0.50,"quantidade":100},{"_id":"idj52","nome":"Donalds Chocolate","descricao":"Sabor chocolate","precovenda":2.50,"precocusto":0.50,"quantidade":100}]`
    );
  }
}
