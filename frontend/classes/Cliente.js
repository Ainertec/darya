...
let VETORDECLIENTESCLASSECLIENTE = [], CONTADORDEENDERECO = 0;
...
async function modalTelaCadastrarouAtualizarCliente(tipo) {
    ...
    if (tipo == 'cadastrar') {
        VETORDECLIENTESCLASSECLIENTE.push(JSON.parse(`{"_id":"-1","name":"", "phone":[], "address":[]}`));
...
async function adicionarDadosNaTabelaTelefoneeEndereco(tipo, id) {
    ...
    if (tipo == "telefone") {
        ...
        $('#tabelatelefone').append(`<tr id="linhatel${tamanhoListaTel - 1}">
            <td class="table-warning"><span class="fas fa-phone"></span> ${document.getElementById('telefonecliente').value}</td>
            <td class="table-warning"><button onclick="removerDadosNaTabelaTelefoneeEndereco('telefone','${document.getElementById('telefonecliente').value}', '${id}', ${tamanhoListaTel - 1});" type="button" class="btn btn-outline-danger btn-sm"><span class="fas fa-trash"></span> Excluir</button></td>
        </tr>`)
    } else if (tipo == "endereco") {
        cliente.address.push(JSON.parse(`{     "_id":"${CONTADORDEENDERECO}",
        ...
        let bairro = BAIRROCLIENTE.data.find(element => element._id == document.getElementById('bairrocidadecliente').value)
        $('#tabelaendereco').append(`<tr id="linhaend${CONTADORDEENDERECO}">
            <td class="table-warning"><span class="fas fa-map-marker-alt"></span> ${document.getElementById('ruacliente').value}</td>
...
function removerDadosNaTabelaTelefoneeEndereco(tipo, telefone, idCliente, posicao) {
    ...
    if (tipo == "telefone") {
        ...
        document.getElementById("tabelatelefone").removeChild(document.getElementById(`linhatel${posicao}`))
    } else if (tipo == "endereco") {
        const enderecoPosition = cliente.address.findIndex(element => element._id == posicao)

        cliente.address.splice(enderecoPosition, 1)
        document.getElementById("tabelaendereco").removeChild(document.getElementById(`linhaend${posicao}`))
    }
}
